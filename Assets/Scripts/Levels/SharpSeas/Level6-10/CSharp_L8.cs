 ...
 * learn about if-else-if statements in C#.
 * 
 * Date: 03/30/2023
...
{
    public class CSharp_L8 : MonoBehaviour
    {
        // Public variables
        ...
        // Private variables
        private readonly double[] GoodPrices = new double[] { 101.74, 98.87, 102.05, 101.74, 114.24 };
        private readonly double[] BadPrices = new double[] { 1152.68, 1.00, 83.00, 101.12, 1010.01 };
        private string declarations;
        private bool displayLog;

        // Use update to check the completion box.
        // Correct shows the correct prices
        // Incorrect shows the incorrect prices
        private void Update()
        {
        ...
        public void MainDriver()
        {
            ...
            public class RuntimeScript : MonoBehaviour
            {
                ...
                void Start()
                {
                    ...
                    string[] nameTest = new string[] { ""Salmon"", ""Halibut"", ""Halibut"", ""Salmon"", ""Salmon"" };
                    double[] weightTest = new double[] { 10, 29, 40, 30, 50.2 };
                    double[] sizeTest = new double[] { 12, 25.50, 25, 25, 22.2 };
                    double[] answers = new double[] { 101.74, 98.87, 102.05, 101.74, 114.24 };
                    int grade = 0;
                    " + declarations + @"
                    ...
                    // Test if the grade is perfect
                    if (grade == 4)
                    {
                        // Display a message and set the completed text box
                        output.text = ""You have successfully sold all the fish at the right price.\n\nAWESOME JOB BIT!!"";
                    ...
                    else
                    {
                        ...
                        output.text = ""Your code failed during one of the test cases. Don't forget to nest your if-else statements."";
        ...
        private string InputModification(string playerCode)
        {
            ...
            if (!newCode.Contains("FishName") || !newCode.Contains("FishWeight") ||
                !newCode.Contains("FishSize") || !newCode.Contains("Price"))
            {
                // Inform the player they need to use certain variables
                programOutput.text = "Sorry, but you must declare and use the variables FishName, FishWeight, FishSize and Price.";
                throw new Exception("Sorry, but you must declare and use the variables FishName, FishWeight, FishSize and Price.");
            }

            // Test that the user used an if-else statement (Will not test for nested, code runs test for that).
            if (!newCode.Contains("if") || !newCode.Contains("else"))
            {
                programOutput.text = "Sorry, but you must use an if-else-if statement for this level.";
                throw new Exception("Sorry, but you must use an if-else-if statement for this level.");
            }
        ...
        // This function, DisplayCorrect, will display customers
        // going to the checkout stand and purchasing fish.
        // The display box will display the correct prices.
        private IEnumerator DisplayCorrect()
        {
            Vector3 startLocation;

            // Loop through each of the NPC Objects
            for (int i = 0; i < 5; ++i)
            {
                // Get the starting location of the NPC and set him/her active
                startLocation = NPC[i].transform.position;
                NPC[i].SetActive(true);

                // Loop through moving the NPC to the check stand
                for (int j = 0; j < 430; ++j)
                {
                    // Move the GoodFood object down by 1
                    NPC[i].transform.Translate(1, 0, 0);

                    // Yield statement allows the NPC to simulate moving through space in frames
                    yield return new WaitForSeconds(0.007f);
                }

                // End of move, set NPC inactive, reset position and show sold price
                decimal sold = Convert.ToDecimal(GoodPrices[i]);
                priceBox.text = sold.ToString("C2"); // Currency formatter
                NPC[i].SetActive(false);
                NPC[i].transform.position = startLocation;
            }
        ...
        // This function, DisplayCorrect, will display customers
        // going to the checkout stand and purchasing fish.
        // The display box will display the incorrect prices.
        private IEnumerator DisplayIncorrect()
        {
            ...
            for (int i = 0; i < 5; ++i)
            {
                ...
                // End of move, set NPC inactive, reset position and show sold price
                decimal sold = Convert.ToDecimal(BadPrices[i]);
                priceBox.text = sold.ToString("C2"); // Currency formatter
                NPC[i].SetActive(false);
                NPC[i].transform.position = startLocation;
            }
