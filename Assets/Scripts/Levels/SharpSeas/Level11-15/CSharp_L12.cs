 ...
 * time Turtles moving across the ground
 * 
...
namespace CSharpLevels
{
    public class CSharp_L12 : MonoBehaviour
    {
        ...
        public List<GameObject> sceneTurtles;
        ...
        public void MainDriver()
        {
            ...
            public class RuntimeScript : MonoBehaviour
            {
                ...
                private GameObject[] turtles = new GameObject[6];
                ...
                void Start()
                {
                    ...
                    // Set each Turtle GameObject.
                    turtles[0] = GameObject.Find(""April"");
                    turtles[1] = GameObject.Find(""Leonardo"");
                    turtles[2] = GameObject.Find(""Raphael"");
                    turtles[3] = GameObject.Find(""Donatello"");
                    turtles[4] = GameObject.Find(""Michelangelo"");
                    turtles[5] = GameObject.Find(""Splinter"");
                ...
                {
                    float[] direction = new float[] { 2f, 1.7f, 1.9f, 1.2f, 1.5f, 1f };
                    Vector3 startPosition = turtles[0].transform.position;
                    ...
                    for (int random = 0; random < 6; ++random)
                    {
                        ...
                        turtles[random].transform.position = startPosition;
                        output.text += ""\r\n""; // New Line for pretty print
                    }
                    ...
                    {
                        output.text += ""Bit, I am sorry to tell you this, but the Turtle times are not correct. Try again"";
         ...
         * and that the player used a do-while for this level. This function will
         * also add in extra code for this level to work.
        ...
        private string InputModification(string playerCode)
        {
            ...
            turtles[random].transform.Translate(direction[random], 0, 0);
            if (turtles[random].transform.position.x > 1375)
            {
                turtles[random].SetActive(false);
                isFinished = true;
            }";
            ...
            if ((newCode.Contains("while") && !newCode.Contains("do")) || newCode.Contains("for"))
            {
                // Both are required due to Unity issues with display
            ...
            if (!newCode.Contains("isFinished"))
            {
                // Inform the player they need to use certain variables
                programOutput.text = "Sorry, but you must declare and use the variable isFinished.";
                throw new Exception("Sorry, but you must declare and use the variable isFinished.");
            }

            // Test that the user used a do-while loop.
            if (!newCode.Contains("do"))
            {
                programOutput.text = "Sorry, but you must use a do-while loop for this level.";
                throw new Exception("Sorry, but you must use a do-while loop for this level.");
            }

            // Test that the user wrote a print-type statement
            if (!newCode.Contains("System.Console.WriteLine"))
            {
                programOutput.text = "Sorry, but you must have a System.Console.WriteLine statement for this level.";
                throw new Exception("Sorry, but you must have a System.Console.WriteLine statement for this level.");
            }

            // Get the starting index of the System.Console.WriteLine statement and the parenthesis
            ...
            // Remove the parenthesis from the System.Console.WriteLine statement
            newCode = newCode.Remove(startCurlyBrace, 1);
            newCode = newCode.Remove(endCurlyBrace, 1);

            // Insert new text after the user's time variable for time units
            newCode = newCode.Insert(endCurlyBrace, " + \" Clock Cycles\"");

            // Replace the System.Console.WriteLine statement with the output variable
            newCode = newCode.Replace("System.Console.WriteLine", "output.text += turtles[random].name + \": \" + ");

            // Put a yield return WaitForSeconds inside the do-while loop for infinite loop checking
            ...
            {
                // Step 3: Insert "yield return WaitForSeconds" after the startCurlyBrace location
                newCode = newCode.Insert(startCurlyBrace + 1, "yield return new WaitForSeconds(0.01f);");
                ...
                // Additional code moves turtles forward by direction each iteration of the loop
                newCode = newCode.Insert(endCurlyBrace, additionalCode);
            }
            else // Single lined do-while loop
            {
                // Step 4: Add in curly braces and the "yield return WaitForSeconds" code
                // Left curlyBrace and the added code
                newCode = newCode.Insert(startIndex + 2, "{yield return new WaitForSeconds(0.01f);");

                // Get an index for while part of the loop
                endCurlyBrace = newCode.IndexOf("while", startIndex);

                // Insert the ending curly brace and additional code to move turtles forward by direction each iteration of the loop
                newCode = newCode.Insert(endCurlyBrace, additionalCode + "}");
            }
        ...
        // This function, TurtleTime, activates the six turtles in the scene
        public void TurtleTime()
        {
            foreach (GameObject turtle in sceneTurtles)
